...
def solve_optin_for_r(r):
    ...
    # Creators' net surplus (for "creator-opt" objective)
    PiC_net = PiC_transfer - phi * (Df_in ** psi)
...
def regime_components(name, A, r, Df, tau_reg):
    prims = compute_primitives(A, B, r, kappa)
    Admin_leak = tau_reg * prims["Roy"]            # − resource
    Creation_cost = phi * (Df ** psi)              # − resource

    # Model-consistent total welfare
    TotalW = prims["CS"] + prims["PiAI"] - Admin_leak - Creation_cost

    # Display components (Π^C is transfer-only; transfers don't enter TotalW)
    PiC_transfer = (1.0 - tau_reg) * r * prims["S"]

    return dict(Regime=name, r=r,
                CS=prims["CS"]*scale_factor,
                PiAI=prims["PiAI"]*scale_factor,
                PiC=PiC_transfer*scale_factor,         # transfer ONLY
...
st.caption("Note: Πᶜ shown is transfer-only ((1−τ)·r·S). Transfers do not enter Total Welfare; only Admin (−τ·r·S) and Creation (−φ·D_f^ψ) are real resource costs.")
